...
# main.py — FastAPI backend for PitchDNA MVP
import os
...
@app.get("/")
def read_root():
    return {"message": "PitchDNA backend is live and serving 🔥"}
@app.get("/umap_data")
def get_umap_data():
    try:
        columns_to_include = [
        ...
        print("🔥 Error in /umap_data:", str(e))
...
def project_umap(pitch: PitchFeatures):
    try:
        ...
        print("📥 Received input features:", input_dict)
        print("📊 Scaled vector (pre-UMAP):", query_vec)
        ...
        print("🎯 UMAP coordinates:", umap_projection[0])

        return {"umap_1": float(umap_projection[0, 0]), "umap_2": float(umap_projection[0, 1])}
    except Exception as e:
        print("🔥 Error in project_umap:", str(e))
        import traceback; traceback.print_exc()
